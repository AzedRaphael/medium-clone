...
function Post({post}:Props) {
  ...
  return (
    <main>
        <Header/>
        <img className="max-w-3xl mx-auto p-5" src={urlFor(post.mainImage).url()!} alt="" />
        ...
        <article className="max-w-3xl mx-auto p-5">
            ...
            <div className="flex items-center space-x-2">
                ...
                <p className="font-extralight text-sm">
                    Blog post by {" "} 
                    <span className="text-green-600"></span>Published at {new Date(post._createdAt).toLocaleString()}
                </p>
            </div>
        ...
        <div className="flex flex-col p-10 my-10 max-w-2xl mx-auto shadow-yellow-500 shadow space-y-2">
            <h3 className="txt-4xl">Comments</h3>
            <hr className="pb-2" />
...
//this returns an array of paths that will be needed
...
//its uses the slugs or url from the paths
export const getStaticProps: GetStaticProps = async ({params})=>{
    const query = `*[_type == "post" && slug.current == $slug][0]{
      ...
      if(!post){
        return {
            // this will force a 404 error page
            notFound:true
        }
      }
      return {
        ...
        //after 60seconds the page will be update the cached version.
        revalidate : 60
      }
}